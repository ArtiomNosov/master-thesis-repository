...
def main() -> int:
    ...
    for path in SVGS:
        ...
        is_thesis_asset = "thesis/assets" in path.as_posix()

        if is_thesis_asset and not is_graphviz:
        ...
        if "thesis/assets" in path.as_posix():
            ...
            if is_graphviz:
                dot_src = path.parent.parent / "diagrams" / f"{path.stem.removesuffix('_marked')}.dot"
                if not dot_src.is_file():
                    errors.append(f"{path.name}: missing source {dot_src.name}")
