...
function sidebar(parent, active) {
  ...
  text(parent, "Управление ML в продакшене", 80, 54, 160, { size: 11, color: "#8EA2BB" });
  text(parent, "МОНИТОРИНГ", 32, 112, 160, { size: 10, style: "Bold", color: "#7890AD" });
  const items = ["Дашборд", "Модели", "Дрейф данных", "Производительность", "Оповещения", "Реестр", "Настройки"];
  ...
  text(parent, "Состояние продакшена", 38, 870, 180, { size: 13, style: "Bold", color: "#FFFFFF" });
  text(parent, "Критичные ML-проблемы показываются с контекстом модели, дрейфа и действий.", 38, 895, 174, {
    size: 11,
    color: "#9FB1CA",
    h: 48,
    lineHeight: 16
...
function topbar(parent, placeholder, right = "Последние 7 дней") {
  ...
  text(parent, "Продакшен", 1248, 32, 72, { size: 12, style: "Bold" });
...
function dashboard(x) {
  const f = makeFrame("01 — Главный MLOps-дашборд", x);
  sidebar(f, "Дашборд");
  topbar(f, "Поиск моделей, признаков, оповещений...");
  header(f, "MLOPS-ДАШБОРД", "Обзор состояния моделей в продакшене", "Отслеживание качества, дрейфа, оповещений, инцидентов и готовности продакшена.", ["Growth ML", "Все модели", "7Д"]);
  metric(f, 296, 232, 258, "Статус системы", "92,4%", "Соответствие SLO, −2,1% к прошлой неделе", "warning");
  ...
  alertCard(f, 1062, 438, 298, "Высокий дрейф данных", "fraud_detection_v2 — оценка дрейфа 0,81", "Критично");
  alertCard(f, 1062, 536, 298, "F1 ниже базы", "churn_prediction_v3 — текущий 0,872", "Предупр.");
  table(f, 296, 696, 1086, ["Модель", "Статус", "F1", "Дрейф", "Задержка", "Владелец"], [
    ["churn_prediction_v3", ["Предупр.", "warning"], "0,872", "0,43", "96 мс", "Н. Орлова"],
    ["fraud_detection_v2", ["Критично", "critical"], "0,781", "0,81", "142 мс", "М. Петров"],
    ["demand_forecast_v1", ["Норма", "healthy"], "0,914", "0,18", "184 мс", "А. Ким"],
    ["credit_risk_v4", ["Норма", "healthy"], "0,928", "0,12", "88 мс", "Д. Иванов"]
  ]);
}
...
function modelDetail(x) {
  ...
  sidebar(f, "Модели");
  topbar(f, "Поиск в churn_prediction_v3...", "Последние 30 дней");
  header(f, "КАРТОЧКА МОДЕЛИ", "churn_prediction_v3", "Классификатор оттока клиентов в продакшене.", ["v3.8.2", "Прод", "База"]);
  ...
  ["Версия 3.8.2", "Продакшен", "Деплой 21 мая", "Датасет retention_2026_05"].forEach((item, i) => {
  ...
  metric(f, 572, 408, 258, "Точность (P)", "0,904", "База 0,908", "healthy");
  metric(f, 848, 408, 258, "Полнота", "0,842", "База 0,884", "warning");
  metric(f, 1124, 408, 258, "ROC-AUC", "0,931", "База 0,951", "warning");
...
function drift(x) {
  ...
  header(f, "МОНИТОРИНГ ДРЕЙФА ДАННЫХ", "Сдвиг распределений: обучение vs продакшен", "Оценка дрейфа по признакам с визуальной критичностью и последними проверками.", ["fraud_detection_v2", "PSI + KS"]);
  ...
  table(f, 296, 704, 1086, ["Признак", "Тип", "Оценка дрейфа", "Статус", "Проверка"], [
    ["transaction_amount", "числовой", "0,81", ["Критично", "critical"], "25 мая, 14:12"],
    ["merchant_category", "категор.", "0,74", ["Критично", "critical"], "25 мая, 14:12"],
    ["device_risk_score", "числовой", "0,66", ["Критично", "critical"], "25 мая, 14:12"],
    ["geo_distance_km", "числовой", "0,49", ["Предупр.", "warning"], "25 мая, 14:12"],
    ["customer_age", "числовой", "0,18", ["Норма", "healthy"], "25 мая, 14:12"]
  ]);
}
...
function performance(x) {
  ...
  header(f, "ПРОИЗВОДИТЕЛЬНОСТЬ МОДЕЛИ", "fraud_detection_v2 — операционные и бизнес-метрики", "Динамика метрик, сравнение версий, матрица ошибок, задержка, пропускная способность и бизнес-эффект.", ["Продакшен", "v2.4.1 vs v2.3.8"]);
  metric(f, 296, 232, 200, "ROC-AUC", "0,902", "Качество модели", "warning");
  metric(f, 514, 232, 200, "Задержка p95", "142 мс", "Ниже жёсткого лимита", "warning");
  metric(f, 732, 232, 200, "Пропускная способность", "1,8k/мин", "Объём предсказаний", "healthy");
  metric(f, 950, 232, 200, "Доля ошибок", "3,4%", "Выше цели", "critical");
  metric(f, 1168, 232, 214, "Бизнес-эффект", "−$42k", "К базе", "critical");
  chart(f, 296, 388, 724, 286, "line");
  table(f, 1040, 388, 342, ["Метрика", "v2.4.1", "v2.3.8"], [
    ["Точность (P)", "0,835", "0,861"],
    ["Полнота", "0,881", "0,842"],
  ...
  text(f, "Заблокировано: $318k", 1044, 758, 260, { size: 13, style: "Bold" });
  text(f, "Ложноположительные: $71k", 1044, 792, 260, { size: 13, style: "Bold", color: C.red });
  text(f, "Пропущено: $42k", 1044, 826, 260, { size: 13, style: "Bold", color: C.red });
  badge(f, "Чистый −$42k", 1044, 858, "critical", 110);
}

function alerts(x) {
  const f = makeFrame("05 — Оповещения и инциденты", x);
  sidebar(f, "Оповещения");
  topbar(f, "Поиск оповещений по модели, владельцу, причине...", "Открытые");
  header(f, "ОПОВЕЩЕНИЯ И ИНЦИДЕНТЫ", "Очередь приоритетных реакций", "Критичность, причина, модель, статус, время и быстрые действия.", ["Новые + расследуются", "Все владельцы"]);
  metric(f, 296, 232, 258, "Новые", "7", "3 требуют назначения", "info");
  metric(f, 572, 232, 258, "Расследуются", "5", "Средний возраст 38 мин", "warning");
  metric(f, 848, 232, 258, "Критичные", "1", "fraud_detection_v2", "critical");
  metric(f, 1124, 232, 258, "Решено сегодня", "12", "MTTR 26 мин", "healthy");
  alertCard(f, 296, 406, 1086, "Высокий дрейф в transaction_amount", "Причина: отклонение распределения в продакшене. Модель: fraud_detection_v2. Время: 25 мая, 14:00 UTC. Статус: новое.", "Критично");
  alertCard(f, 296, 510, 1086, "Полнота ниже порога", "Причина: дрейф модели после изменения трафика. Модель: churn_prediction_v3. Время: 25 мая, 13:48 UTC. Статус: расследуется.", "Предупр.");
  alertCard(f, 296, 614, 1086, "Задержка p95 выше порога", "Причина: насыщение сервиса предсказаний. Модель: demand_forecast_v1. Время: 25 мая, 13:21 UTC. Статус: расследуется.", "Предупр.");
  table(f, 296, 744, 1086, ["Инцидент", "Модель", "Критичность", "Статус", "Владелец"], [
    ["Несоответствие схемы", "fraud_detection_v2", ["Критично", "critical"], "Новый", "Не назначен"],
    ["Нарушение порога качества", "churn_prediction_v3", ["Предупр.", "warning"], "Расследуется", "Н. Орлова"],
    ["Насыщение сервиса", "demand_forecast_v1", ["Предупр.", "warning"], "Расследуется", "А. Ким"]
  ]);
}
...
function registry(x) {
  ...
  sidebar(f, "Реестр");
  topbar(f, "Поиск в реестре по модели, версии, датасету...", "Все проекты");
  header(f, "РЕЕСТР МОДЕЛЕЙ", "Модели, версии, датасеты, владельцы и действия деплоя", "Управление жизненным циклом от обучения до деплоя, отката и сравнения версий.", ["Все среды", "Одобренные"]);
  metric(f, 296, 232, 258, "Зарегистрировано", "28", "12 активны в продакшене", "info");
  metric(f, 572, 232, 258, "Одобренные версии", "9", "Прошли валидацию", "healthy");
  ...
  table(f, 296, 406, 1086, ["Модель/версия", "Статус", "Обучение", "Деплой", "Датасет", "Владелец"], [
    ["churn_prediction_v3 / v3.8.2", ["Предупр.", "warning"], "18 мая", "21 мая", "retention_2026_05", "Н. Орлова"],
    ["fraud_detection_v2 / v2.4.1", ["Критично", "critical"], "12 мая", "19 мая", "fraud_txn_v9", "М. Петров"],
    ["demand_forecast_v1 / v1.9.0", ["Норма", "healthy"], "3 мая", "9 мая", "demand_weekly_v14", "А. Ким"],
    ["credit_risk_v4 / v4.2.5", ["Одобрено", "healthy"], "22 мая", "Не развёрнуто", "credit_app_2026_q2", "Д. Иванов"],
    ["price_elasticity_v1 / v1.3.4", ["Архив", "neutral"], "12 мар", "1 апр", "pricing_history_v7", "С. Ли"]
  ]);
  ["Подробнее", "Сравнить версии", "Деплой", "Откат"].forEach((label, i) => button(f, label, 296 + i * 144, 764, 130, i === 2));
}
...
async function main() {
  ...
  figma.closePlugin("Созданы экраны MLOps-дашборда ModelPulse.");
}
