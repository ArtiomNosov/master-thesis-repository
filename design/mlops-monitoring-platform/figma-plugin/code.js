...
function dashboard(x) {
  ...
  header(f, "ПАНЕЛЬ МОНИТОРИНГА", "Обзор состояния моделей в промышленной среде", "Отслеживание качества, дрейфа, оповещений и готовности промышленной среды.", ["«Подбор ИТ»", "Все модели", "7 дней"]);
  ...
  alertCard(f, 1062, 438, 298, "Высокий дрейф признаков", "скоринг_соответствия_2 — оценка дрейфа 0,81", "Критично");
  alertCard(f, 1062, 536, 298, "Гармоническая мера ниже базы", "ранжирование_резюме_3 — текущий 0,872", "Предупр.");
  table(f, 296, 696, 1086, ["Модель", "Статус", "Гарм. мера", "Дрейф", "Задержка", "Владелец"], [
    ["ранжирование_резюме_3", ["Предупр.", "warning"], "0,872", "0,43", "96 мс", "Н. Орлова"],
    ["скоринг_соответствия_2", ["Критично", "critical"], "0,781", "0,81", "142 мс", "М. Петров"],
    ["сортировка_откликов_1", ["Норма", "healthy"], "0,914", "0,18", "184 мс", "А. Ким"],
    ["извлечение_навыков_4", ["Норма", "healthy"], "0,928", "0,12", "88 мс", "Д. Иванов"]
  ]);
}
...
function modelDetail(x) {
  ...
  sidebar(f, "Модели");
  topbar(f, "Поиск в ранжирование_резюме_3...", "Последние 30 дней");
  header(f, "КАРТОЧКА МОДЕЛИ", "ранжирование_резюме_3", "Модель ранжирования резюме в промышленной среде.", ["вер. 3.8.2", "Пром", "База"]);
  ...
  ["Версия 3.8.2", "Промышленная", "Развёртывание 21 мая", "отклики_2026_05"].forEach((item, i) => {
...
function drift(x) {
  ...
  header(f, "МОНИТОРИНГ ДРЕЙФА ДАННЫХ", "Сдвиг распределений: обучение и промышленная среда", "Оценка дрейфа по признакам с визуальной критичностью.", ["скоринг_соответствия_2", "ИС + КС"]);
  ...
  table(f, 296, 704, 1086, ["Признак", "Тип", "Оценка дрейфа", "Статус", "Проверка"], [
    ["совпадение_навыков", "числовой", "0,81", ["Критично", "critical"], "25 мая, 14:12"],
    ["источник_отклика", "категор.", "0,74", ["Критично", "critical"], "25 мая, 14:12"],
    ["опыт_лет", "числовой", "0,66", ["Критично", "critical"], "25 мая, 14:12"],
    ["зарплатные_ожидания", "числовой", "0,49", ["Предупр.", "warning"], "25 мая, 14:12"],
    ["число_релевантных_проектов", "числовой", "0,18", ["Норма", "healthy"], "25 мая, 14:12"]
  ]);
}
...
function performance(x) {
  ...
  header(f, "ПРОИЗВОДИТЕЛЬНОСТЬ МОДЕЛИ", "скоринг_соответствия_2 — качество ранжирования", "Динамика качества, сравнение версий, матрица релевантности и эффект для рекрутера.", ["Промышленная среда", "вер. 2.4.1 и 2.3.8"]);
  ...
  metric(f, 1168, 232, 214, "Экономия проверки", "+205 мин", "К базе", "healthy");
  chart(f, 296, 388, 724, 286, "line");
  ...
  text(f, "Заблокировано: 318 мин", 1044, 758, 260, { size: 13, style: "Bold" });
  text(f, "Ложноположительные: 71 мин", 1044, 792, 260, { size: 13, style: "Bold", color: C.red });
  text(f, "Пропущено: 42 мин", 1044, 826, 260, { size: 13, style: "Bold", color: C.red });
  badge(f, "Чистый +205 мин", 1044, 858, "healthy", 110);
}
...
function alerts(x) {
  ...
  metric(f, 848, 232, 258, "Критичные", "1", "скоринг_соответствия_2", "critical");
  metric(f, 1124, 232, 258, "Решено сегодня", "12", "Среднее время восстановления 26 мин", "healthy");
  alertCard(f, 296, 406, 1086, "Высокий дрейф в совпадение_навыков", "Причина: отклонение распределения. Модель: скоринг_соответствия_2. Время: 25 мая, 14:00.", "Критично");
  alertCard(f, 296, 510, 1086, "Полнота ниже порога", "Причина: дрейф модели. Модель: ранжирование_резюме_3. Время: 25 мая, 13:48.", "Предупр.");
  alertCard(f, 296, 614, 1086, "Задержка 95-го перцентиля", "Причина: насыщение сервиса. Модель: сортировка_откликов_1. Время: 25 мая, 13:21.", "Предупр.");
  table(f, 296, 744, 1086, ["Инцидент", "Модель", "Критичность", "Статус", "Владелец"], [
    ["Несоответствие схемы", "скоринг_соответствия_2", ["Критично", "critical"], "Новый", "Не назначен"],
    ["Нарушение порога качества", "ранжирование_резюме_3", ["Предупр.", "warning"], "Расследуется", "Н. Орлова"],
    ["Насыщение сервиса", "сортировка_откликов_1", ["Предупр.", "warning"], "Расследуется", "А. Ким"]
  ]);
}
...
function registry(x) {
  ...
  table(f, 296, 406, 1086, ["Модель/версия", "Статус", "Обучение", "Развёртывание", "Набор данных", "Владелец"], [
    ["ранжирование_резюме_3 / вер. 3.8.2", ["Предупр.", "warning"], "18 мая", "21 мая", "отклики_2026_05", "Н. Орлова"],
    ["скоринг_соответствия_2 / вер. 2.4.1", ["Критично", "critical"], "12 мая", "19 мая", "резюме_вакансии_9", "М. Петров"],
    ["сортировка_откликов_1 / вер. 1.9.0", ["Норма", "healthy"], "3 мая", "9 мая", "профили_кандидатов_14", "А. Ким"],
    ["извлечение_навыков_4 / вер. 4.2.5", ["Одобрено", "healthy"], "22 мая", "Не развёрнуто", "вакансии_заявки_2026_2кв", "Д. Иванов"],
    ["парсинг_резюме_1 / вер. 1.3.4", ["Архив", "neutral"], "12 мар", "1 апр", "резюме_архив_7", "С. Ли"]
  ]);
