...
Архитектурный подход: целью работы является проектирование модуля сопоставления «вакансия–резюме» на основе семантических представлений текста и разработка архитектуры ранжирования кандидатов для интеграции в ATS. Исследовано в работе: в качестве сопутствующих задач выполнены определение воспроизводимой методики оценки качества ранжирования (по метрикам NDCG, Precision@K и др.) и расчет практической эффективности реализованного прототипа. Методологическая база исследования включает современные нейросетевые модели контекстных представлений (BERT, Sentence-BERT), подходы на базе би-энкодера и сиамских архитектур для поиска по семантическим вложениям, а также передовые практики индустриальных систем поиска талантов [1, 6, 8, 17].
...
1. Реализовано в работе: разметка глобальной релевантности пар «вакансия–резюме» для последующего обучения ранжирующих моделей и контроля качества по NDCG.
2. Концептуальный компонент: детализированная разметка терминов на уровне выделения текстовых фрагментов (span-level) с типизацией скиллов (hard, soft, knowledge) для построения интерпретируемой выдачи.
3. Архитектурное допущение: косвенная разметка посредством накопления поведенческих сигналов рекрутеров (отказы, приглашения на собеседование) рассматривается как элемент промышленной эксплуатации алгоритма, но в настоящей работе не проверяется как онлайн-механизм.
...
Рисунок 1 — Диаграмма контейнеров C4 модуля семантического ранжирования и Reqcore. Зеленые элементы обозначают компоненты, реализованные в работе; светло-зеленые — данные и артефакты, исследованные в рамках подготовки корпуса; темно-зеленые — экспериментально исследованные элементы модели и оценки ранжирования (ranking); оранжевые пунктирные рамки показывают архитектурные допущения будущего масштабирования; фиолетовая пунктирная рамка обозначает концептуальный компонент, не проверенный как результат текущей работы. На диаграмме технические идентификаторы (например, `application.score`, `NDCG@K`, `/score`) даны в скобках после русских названий.

**Спецификация программного интерфейса (API) и модулей интеграции с ядром ATS**

Реализовано в работе: API-модуль создан в скрипте API-сервера (`experiments/scripts/11_api_server.py`) как микросервис FastAPI `ATS Semantic Ranking Engine API` версии `1.0.0`. В текущей реализации сервер использует Pydantic-схемы `CandidatePayload` и `SearchPayload`, временный индекс кандидатов в памяти (`CANDIDATE_INDEX`) и модуль ранжирования (`ATSRanker`), загружаемый при старте приложения. Кандидатский профиль передается в API в уже подготовленном текстовом формате `model_input`; отдельные публичные маршруты для самостоятельного построения эмбеддингов вакансии или резюме в коде не реализованы.
...
Рисунок 2 — Схема потока анализа заявки. Зеленые элементы с пометкой «Реализовано в работе» отражают фактически реализованные участки Reqcore и сервиса FastAPI; темно-зеленая пометка «Экспериментально исследовано» показывает вычисление косинусной оценки (cosine score) модулем ранжирования (`ATSRanker`), качество которого проверялось на val/test split; синяя пометка «Архитектурный подход» обозначает выбранное разделение LLM-scoring и bi-encoder scoring (ветка `provider === 'biencoder'`, маршрут `/score`) как проектную схему интеграции.

Реализовано в работе: в текущем контуре затронуты следующие файлы: `ats/reqcore/server/api/applications/[id]/analyze.post.ts` как основная точка маршрутизации; `ats/reqcore/server/utils/ai/scoring.ts` как движок LLM-scoring; `ats/reqcore/server/utils/ai/provider.ts` как адаптер провайдера (`provider`); `ats/reqcore/server/database/schema/app.ts` как место определения таблиц оценок по критериям (`criterion_score`), аудита анализа (`analysis_run`) и поля оценки заявки (`application.score`); `experiments/scripts/11_api_server.py` как Python-микросервис bi-encoder scoring. Именно эти файлы образуют фактическую интеграционную поверхность между Reqcore и модулем ML-ранжирования.
...
Архитектурный подход: контур разработки ML-кода разделен на офлайн- и онлайн-контуры в соответствии с передовой практикой систем талантов [17]. Офлайн-модули собирают, подготавливают данные и проводят обучение; онлайн-система занимается инференсом эмбеддингов и передачей API-команд. Реализовано в работе: совокупный программный комплекс (ATS AI Engine) состоит из 17 взаимосвязанных Python-скриптов, исполняемых в строго определенном порядке нумерации: от модулей нормализации и извлечения сущностей (скрипты 05–07), через модули обучения и инференса модели (08–09), ядро ранжирования и API-микросервис (10–11), до серии экспериментальных бенчмарков (12–17). Все зависимости фиксированы через стандартные библиотеки (`sentence-transformers`, `scikit-learn`, `fastapi`, `uvicorn`), что обеспечивает полную воспроизводимость результатов.
...
Реализовано в работе: при реализации контура векторного кодирования фактически используется только модель `cointegrated/rubert-tiny2`. Это подтверждается обучающим скриптом `experiments/scripts/09_train_biencoder.py`, где аргумент `--model_name` имеет значение по умолчанию `cointegrated/rubert-tiny2`, а также сохраненным артефактом `experiments/models/bi_encoder_rubert_tiny2`, в `README.md` которого указан `base_model: cointegrated/rubert-tiny2`. Архитектурное допущение: возможное применение `ru-en-RoSBERTa` рассматривается как направление дальнейшего сравнения, но не как результат текущего эксперимента.
...
Архитектурное допущение: экономическая оценка внедрения не трактуется как результат production billing export или фактический замер рабочего времени HR-специалистов в промышленной ATS. Она строится на сочетании исследованных в работе технических метрик и явно заданных расчетных допущений, необходимых для воспроизводимого экономического сценария.

Исследовано в работе: в качестве технического источника времени используется полный прогон ranking-оценки на `data/splits/test.tsv`: файл `experiments/results/full_test_ranking_metrics_stdout.txt` фиксирует `521` обработанную пару `вакансия-резюме` и `runtime_seconds = 3.870512`. Отсюда среднее модельное время на одну заявку:

```text
T_model = 3.870512 / 521 = 0.007429 с = 0.000124 мин.
```

Архитектурное допущение: для ручного сценария в расчет введены явные допущения, поскольку в production-логах Reqcore отсутствует независимый замер рабочего времени HR-специалиста. В презентации ВКР для ручного скрининга уже зафиксирован ориентир `45` минут на цикл из `20` заявок; для расчета принято, что один такой цикл содержит ровно `20` заявок. Время ручной обработки одной заявки:

```text
T_human = 45 / 20 = 2.25 мин.
```

Архитектурный подход: после внедрения модуль не принимает кадровое решение автономно: он сортирует заявки и передает результат HR-специалисту. Для контрольной проверки ранжированного списка принята верхняя граница из презентационного сценария: `4` минуты на `20` заявок, то есть:

```text
T_check_hr = 4 / 20 = 0.20 мин.
```

Сравнение по времени и качеству приведено в таблице. Точность ручной классификации относительно исторических HR-меток принимается равной `1.0000`, так как сами HR-события являются источником разметки `label`. Это не утверждение об абсолютной объективности человека, а техническое свойство выбранного офлайн-дизайна эксперимента. Экспериментально исследовано: для модели используется финальная test-метрика `best accuracy = 0.9750`, приведенная выше в разделе экспериментальной оценки.
...
Архитектурное допущение: для денежного расчета заданы следующие прозрачные параметры сценария: часовая ставка HR-специалиста `600` руб./ч; стоимость машинного времени CPU-инстанса `30` руб./ч; инженерная ставка для внедрения и сопровождения `1500` руб./ч; разовое внедрение готового прототипа в пилотный контур Reqcore занимает `16` инженерных часов; ежемесячное сопровождение занимает `2` инженерных часа. Реализовано в работе: стоимость первоначального обучения модели также включена в затраты на внедрение на основе внешнего замера команды обучения `6254.096` с, что при ставке `30` руб./ч дает `52.12` руб.
...
Реализовано в работе: прототип решает задачу семантического сопоставления «вакансия-резюме» через би-энкодер `cointegrated/rubert-tiny2`, жесткую фильтрацию по metadata и косинусное ранжирование отфильтрованного кандидатского пула. Практический результат ограничен фактически реализованным контуром: подготовка связанных пар, обучение би-энкодера, сохранение модели, in-memory API и brute-force cosine scoring в `ATSRanker`. Экспериментально исследовано: после повторной подготовки данных и fine-tuning метрики разделения пар на `val/test` существенно выросли относительно базовой модели; на полном `test.tsv` по группам вакансий также получены ranking-метрики: NDCG@3 `1.0000` на `33` NDCG-пригодных вакансиях, Precision@3 `0.9958`, Recall@3 `0.9824` и MRR `1.0000` на `119` вакансиях с хотя бы одним positive. Эти результаты подтверждают полезность доменной настройки для текущего корпуса, но ограничены разреженностью test split. Архитектурное допущение: масштабирование через ANN/Vector DB, LTR/RankNet, fair ranking и промышленное нагрузочное тестирование в текущей реализации отсутствуют и должны рассматриваться как будущая работа.
