...
Модуль оценивался в формате офлайн-валидации на пересобранном наборе данных, сформированном из первоисточников `data/super_job`. После очистки и пересборки связанных пар итоговая выборка составила `5 218` примеров, из которых `4 176` использовались для обучения, `521` для валидации и `521` для финального тестирования. Основной сравнительный протокол включает ровно три модели: BM25 lexical ranking baseline без обучения; fine-tuned cross-encoder `cointegrated/rubert-tiny2`, обучаемый на `train.tsv` и валидируемый на `val.tsv`; fine-tuned bi-encoder `experiments/models/bi_encoder_rubert_tiny2`, выступающий proposed approach проекта.
