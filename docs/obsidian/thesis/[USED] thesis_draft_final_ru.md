...
Архитектурный подход: формула top-K pre-ranking показывает экономический смысл bi-encoder как первого этапа в проектной схеме, а не измеренный production-эффект. Если полный LLM scoring выполняется для всех `N` заявок, то:
...
Концептуальный компонент: LLM scoring дает более детальное объяснимое оценивание по критериям, но имеет переменную стоимость за каждый candidate/job вызов и зависит от внешнего API. Bi-encoder scoring в проектной схеме дешевле по внешним API-платежам и лучше подходит для первичного top-K отбора, но требует локального обслуживания модели и не заменяет полноценную экспертную или LLM-интерпретацию причин соответствия кандидата.
