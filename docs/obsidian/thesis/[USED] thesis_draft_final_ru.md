...
Сравнение применимости подходов для темы работы приведено в таблице.

| Семейство методов | Основной сигнал ранжирования | Преимущество в ATS-сценарии | Ограничение относительно BERT-подхода | Роль в данной работе |
|---|---|---|---|---|
| Векторная модель и TF-IDF | Совпадение взвешенных терминов и геометрическая близость разреженных векторов | Простота, интерпретируемость, низкая вычислительная стоимость | Не моделирует контекст и синонимию профессиональных формулировок | Базовая точка аналитического сравнения и кандидат для отдельного воспроизводимого baseline-теста |
| BM25 и вероятностные модели IR | Вероятностно мотивированная оценка совпадения терминов с учетом длины документа | Устойчивый классический baseline для неоднородных текстов вакансий и резюме | Остается зависимым от лексического пересечения и не обучается на доменных парах без дополнительных надстроек | Обязательный ориентир для будущего воспроизводимого сравнения |
| Ссылочные и графовые методы (PageRank) | Важность узла в графе ссылок или взаимодействий | Полезны при наличии богатой сети связей между объектами | Для пары «вакансия-резюме» нет естественного ссылочного графа, поэтому метод не решает основную задачу сопоставления текста | Не является центральной базовой линией; возможен как вспомогательный источник признаков в будущем |
| Классические LTR-модели (RankNet, LambdaMART, градиентный бустинг) | Обученная функция ранжирования по множеству признаков и меткам релевантности | Могут объединять текстовые, структурные и поведенческие признаки и оптимизироваться под NDCG | Требуют подготовленных списков выдачи, градуированной разметки и отдельного обучения переранжировщика | Рассматриваются как следующий этап после bi-encoder recall/scoring |
| BERT/Sentence-BERT bi-encoder | Контекстные плотные эмбеддинги вакансии и резюме, сравниваемые по cosine similarity | Учитывает семантическую близость при неполном лексическом совпадении, допускает fine-tuning на HR-событиях | Более высокая вычислительная стоимость, меньшая прозрачность, зависимость от качества разметки и риск переноса смещений корпуса | Выбран как основной реализованный подход для семантического сопоставления в прототипе |

Таким образом, выбор BERT/Sentence-BERT в данной работе обусловлен не отказом от классического информационного поиска, а спецификой предметной области: при подборе персонала существенную роль играют смысловые соответствия между обязанностями, навыками и профессиональным опытом, которые часто не выражены одинаковыми словами. Классические методы остаются важными для baseline-сравнения, фильтрации и будущего LTR-переранжирования, однако основной реализованный модуль ориентирован на семантическое сопоставление русскоязычных текстов и проверку того, улучшает ли доменное дообучение качество числового ранжирования на исторических парах `вакансия-резюме`.
