...
Современный рекрутмент характеризуется высокой нагрузкой на этапе первичного отбора: в корпоративной практике на одну вакансию могут приходиться сотни откликов, что делает инструменты автоматизированного ранжирования и фильтрации кандидатов ключевым элементом инфраструктуры найма [2].
...
Таблица 2.3 – Спецификация API-маршрутов FastAPI-сервиса ранжирования

| Адрес ресурса | HTTP-метод | Схема входных данных | Описание поведения | Формат ответа |
|---|---|---|---|---|
...
**2.4. Интеграция Reqcore с модулем ML-ранжирования**
...
Таблица 3.2 – Параметры сохранённой модели cointegrated/rubert-tiny2
...
Таблица 3.3 – Гиперпараметры и фактические показатели обучения
...
Семантический этап реализован в методе `search(vacancy_text, candidate_pool, filters=None, top_k=10)`. После фильтрации модуль извлекает поле `model_input` из оставшихся кандидатов, кодирует текст вакансии и тексты кандидатов на лету, вычисляет `util.cos_sim`, выбирает `top_k` через `torch.topk` и формирует JSON-ответ. Ключевые операции сводятся к фильтрации пула, пакетному кодированию текстов и выбору top_k по косинусному сходству; полный фрагмент реализации приведён в листинге А.1.
...
Таблица 3.4 – Ranking-метрики на полном test split
...
**4.2. Выполнение интеграции созданного модуля в целевую ATS-систему (или её тестовый контур)**
...
**4.3. Экономическая оценка LLM scoring и bi-encoder scoring**
...
Используемые публичные константы цен (пересчёт в рубли по курсу ЦБ РФ 92,5 руб./USD на 27.05.2026): для `gpt-4o-mini`, который указан как default в `ai_config`, официально опубликованная цена составляет 13,88 руб. за 1 млн input tokens и 55,50 руб. за 1 млн output tokens; для актуального класса `gpt-5.4-mini` на публичной странице OpenAI API Pricing указано 69,38 руб. за 1 млн input tokens, 6,94 руб. за 1 млн cached input tokens и 416,25 руб. за 1 млн output tokens; для `gpt-5.4-nano` — 18,50 руб., 1,85 руб. и 115,63 руб. соответственно. В расчетах ниже cached input не используется, так как факт применения prompt caching для scoring-запросов в коде и логах не подтвержден.
...
Таблица 4.3 – Сравнительная оценка стоимости LLM scoring

| Модель | Формула | Оценочная стоимость / заявку (руб.) | Оценочная стоимость / 100 заявок (руб.) |
|---|---|---:|---:|
| `gpt-4o-mini` | `(3000 / 1e6) * 13.875 + (700 / 1e6) * 55.5` | 0,08 | 8,05 |
| `gpt-5.4-mini` | `(3000 / 1e6) * 69.375 + (700 / 1e6) * 416.25` | 0,50 | 49,95 |
| `gpt-5.4-nano` | `(3000 / 1e6) * 18.5 + (700 / 1e6) * 115.625` | 0,14 | 13,64 |
...
Например, при `N = 100` и `K = 10` estimate-based сокращение внешних LLM scoring calls составляет `90%`: вместо 100 LLM-вызовов выполняется 10. Для `gpt-4o-mini` при указанном выше токенном допущении это меняет оценочную стоимость с 8,05 руб. до 0,80 руб. за 100 applications; для `gpt-5.4-mini` — с 49,95 руб. до 5,00 руб.; для `gpt-5.4-nano` — с 13,64 руб. до 1,36 руб. (пересчёт по курсу 92,5 руб./USD). Эти значения являются расчетным сценарием, а не результатом production billing export.
...
**4.4. Офлайн-валидация bi-encoder-scoring в Reqcore**
...
Таблица 4.4 – Уровни офлайн-валидации bi-encoder scoring

| Уровень оценки | Что измеряется | Артефакты |
|---|---|---|
| Классификация на уровне пары | Насколько хорошо модель отделяет релевантные пары от нерелевантных | `train.tsv`, `val.tsv`, `test.tsv` |
| Ранжирование на уровне вакансии | Насколько хорошо модель ранжирует кандидатов внутри одной вакансии | группировка `test.tsv` по `vacancy_id` |
...
**Приложение А. Фрагмент реализации метода search класса ATSRanker**

Листинг А.1 – Семантический этап ранжирования в ATSRanker
...
[1] Rosenberger J., Wolfrum L., Weinzierl S., Kraus M., Zschech P. CareerBERT: сопоставление резюме с вакансиями ESCO в общем пространстве вложений для универсальных рекомендаций по подбору персонала [CareerBERT: Matching Resumes to ESCO Jobs in a Shared Embedding Space for Generic Job Recommendations] // Expert Systems with Applications. – 2025. – arXiv:2503.02056.

[2] Fabris A., Rus C., Saldivar J., Gatzioura A., Biega A.J., Castillo C. Приводит ли справедливое ранжирование к справедливым результатам найма? Исследование вмешательств, интерфейсов и взаимодействий [Does fair ranking lead to fair recruitment outcomes? A study of interventions, interfaces, and interactions] // Information Processing & Management. – 2026. – Vol. 63. – 104506.

[3] IBM Research. PROSPECT: система отбора кандидатов для рекрутмента [PROSPECT: A system for screening candidates for recruitment] // Proc. of the 19th ACM Int. Conf. on Information and Knowledge Management (CIKM). – 2010.

[4] IBM Research. Сопоставление резюме и вакансий с помощью глубокой сиамской нейросети [Matching Resumes to Jobs via Deep Siamese Network] // Proc. of the 2018 World Wide Web Conference (WWW). – 2018.

[5] Devlin J., Chang M.-W., Lee K., Toutanova K. BERT: предобучение глубоких двунаправленных трансформеров для понимания естественного языка [BERT: Pre-training of Deep Bidirectional Transformers for Language Understanding]. – arXiv:1810.04805.

[6] Reimers N., Gurevych I. Sentence-BERT: вложения предложений с использованием сиамских BERT-сетей [Sentence-BERT: Sentence Embeddings using Siamese BERT-Networks]. – arXiv:1908.10084.

[7] Vaswani A., Shazeer N., Parmar N., Uszkoreit J., Jones L., Gomez A.N., Kaiser Ł., Polosukhin I. Внимание — это всё, что вам нужно [Attention is All You Need] // Advances in Neural Information Processing Systems 30 (NeurIPS). – 2017.

[8] Liu T.-Y. Обучение ранжированию для информационного поиска [Learning to Rank for Information Retrieval] // Foundations and Trends in Information Retrieval. – 2009. – Vol. 3, No. 3. – P. 225–331.

[9] Burges C., Shaked T., Renshaw E., Lazier A., Deeds M., Hamilton N., Hullender G. Обучение ранжированию с использованием градиентного спуска [Learning to Rank using Gradient Descent] // Proc. of the 22nd Int. Conf. on Machine Learning (ICML). – 2005. – (RankNet).

[10] Järvelin K., Kekäläinen J. Оценка методов IR на основе накопленного gain [Cumulated Gain-based Evaluation of IR Techniques] // ACM Transactions on Information Systems. – 2002. – Vol. 20, No. 4. – P. 422–446.

[11] Robertson S., Zaragoza H. Вероятностная relevance framework: BM25 и дальнейшее развитие [The Probabilistic Relevance Framework: BM25 and Beyond] // Foundations and Trends in Information Retrieval. – 2009. – Vol. 3, No. 4. – P. 333–389.

[12] Wang J., Yi X., Guo R. [и др.]. Milvus: специализированная система управления векторными данными [Milvus: A Purpose-Built Vector Data Management System] // Proc. of the 2021 ACM SIGMOD Int. Conf. on Management of Data. – 2021.

[13] Subramanya S.J., Devvrit, Kadekodi R., Krishaswamy R., Simhadri H.V. DiskANN: быстрый и точный поиск ближайших соседей среди миллиарда точек на одном узле [DiskANN: Fast Accurate Billion-point Nearest Neighbor Search on a Single Node] // Advances in Neural Information Processing Systems 32 (NeurIPS). – 2019.

[14] Chen Q., Zhao B., Wang H. [и др.]. SPANN: высокоэффективный approximate nearest neighbor search в миллиардном масштабе [SPANN: Highly-efficient Billion-scale Approximate Nearest Neighbor Search] // Advances in Neural Information Processing Systems 34 (NeurIPS). – 2021.

[15] Gong Z., Zeng Y., Chen L. Ускорение ANN-поиска в иерархических графах: эффективная навигация по уровням с shortcuts [Accelerating Approximate Nearest Neighbor Search in Hierarchical Graphs: Efficient Level Navigation with Shortcuts] // Proc. of the VLDB Endowment. – 2025. – Vol. 18, No. 10. – P. 3518–3530.

[16] Singh A., Joachims T. Обучение политик для справедливости в ранжировании [Policy Learning for Fairness in Ranking] // Advances in Neural Information Processing Systems 32 (NeurIPS). – 2019.

[17] Li J., Xu B., Chen Z. [и др.]. Улучшение ранжирования talent search с role-aware expert mixtures и LLM-описаниями вакансий [Enhancing Talent Search Ranking with Role-Aware Expert Mixtures and LLM-based Fine-Grained Job Descriptions] // Proc. of the 2025 Conf. on Empirical Methods in Natural Language Processing: Industry Track. – 2025. – P. 185–194.

[18] Senger E., Zhang M., van der Goot R., Plank B. Анализ рынка труда на основе глубокого обучения: обзор извлечения и классификации навыков из вакансий [Deep Learning-based Computational Job Market Analysis: A Survey on Skill Extraction and Classification from Job Postings] // Proc. of NLP4HR Workshop. – 2024. – P. 1–15.

[19] Snegirev A., Tikhonova M., Maksimova A., Fenogenova A., Abramov A. Исследование русскоязычных embedder-моделей: бенчмарк ruMTEB и проектирование русской embedding-модели [The Russian-focused embedders’ exploration: ruMTEB benchmark and Russian embedding model design] // Proc. of the 2025 Conf. of the Nations of the Americas Chapter of the ACL (NAACL). – 2025. – P. 236–254.

[20] Zmitrovich D., Abramov A., Kalmykov A. [и др.]. Семейство предобученных трансформерных языковых моделей для русского языка [A Family of Pretrained Transformer Language Models for Russian]. – arXiv:2309.10931.

[21] Gallegos I.O., Rossi R.A., Barrow J. [и др.]. Смещение и справедливость в больших языковых моделях: обзор [Bias and Fairness in Large Language Models: A Survey] // Computational Linguistics. – 2024. – Vol. 50, No. 3. – P. 1097–1179.

[22] Wilson K., Caliskan A. Гендерное, расовое и пересекающееся смещение при отборе резюме через retrieval языковых моделей [Gender, Race, and Intersectional Bias in Resume Screening via Language Model Retrieval]. – arXiv:2407.20371.

[23] Tang F., Zhu R., Yao F., Wang J., Luo L., Li B. Объяснимые рекомендации «человек–вакансия»: вызовы, подходы и сравнительный анализ [Explainable person–job recommendations: challenges, approaches, and comparative analysis] // Frontiers in Artificial Intelligence. – 2025. – Vol. 8. – 1660548.

[24] Zhang M., Jensen K.N., Sonniks S.D., Plank B. SKILLSPAN: извлечение hard- и soft skills из англоязычных вакансий [SKILLSPAN: Hard and Soft Skill Extraction from English Job Postings] // Proc. of the 2022 Conf. of the North American Chapter of the ACL (NAACL). – 2022. – P. 4962–4984.