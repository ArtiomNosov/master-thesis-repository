...
Как проектное допущение дальнейшего развития, экономическая оценка внедрения не трактуется как результат production billing export или фактический замер рабочего времени HR-специалистов в промышленной ATS. Она строится на сочетании исследованных в работе технических метрик и явно заданных расчетных допущений, необходимых для воспроизводимого экономического сценария.

В эмпирической части работы в качестве технического источника времени используется полный прогон ranking-оценки на `data/splits/test.tsv`: файл `experiments/results/full_test_ranking_metrics_stdout.txt` фиксирует `521` обработанную пару `вакансия-резюме` и `runtime_seconds = 3.870512`. Отсюда среднее модельное время на одну заявку:

```text
T_model = 3.870512 / 521 = 0.007429 с = 0.000124 мин.
```

Как проектное допущение дальнейшего развития, для ручного сценария в расчет введены явные допущения, поскольку в production-логах Reqcore отсутствует независимый замер рабочего времени HR-специалиста. В презентации ВКР для ручного скрининга уже зафиксирован ориентир `45` минут на цикл из `20` заявок; для расчета принято, что один такой цикл содержит ровно `20` заявок. Время ручной обработки одной заявки:

```text
T_human = 45 / 20 = 2.25 мин.
```

С точки зрения целевой архитектуры, после внедрения модуль не принимает кадровое решение автономно: он сортирует заявки и передает результат HR-специалисту. Для контрольной проверки ранжированного списка принята верхняя граница из презентационного сценария: `4` минуты на `20` заявок, то есть:

```text
T_check_hr = 4 / 20 = 0.20 мин.
```

Сравнение по времени и качеству приведено в таблице. Точность ручной классификации относительно исторических HR-меток принимается равной `1.0000`, так как сами HR-события являются источником разметки `label`. Это не утверждение об абсолютной объективности человека, а техническое свойство выбранного офлайн-дизайна эксперимента. На отложенной выборке показано, что для модели используется финальная test-метрика `best accuracy = 0.9750`, приведенная выше в разделе экспериментальной оценки.
...
Как проектное допущение дальнейшего развития, количество заявок за расчетный месяц принято равным `521`, то есть объему отложенного test split. Такой выбор не переносит тестовый набор в production как факт нагрузки, но позволяет сделать воспроизводимый расчет на тех же данных, на которых измерены качество и время. Тогда экономия времени по формуле:

```text
Экономия времени на одной заявке = Время человека на одну заявку - Время модели на одну заявку
Экономия времени за период = Экономия времени на одной заявке * Количество заявок за период
```

Подстановка значений:

```text
2.25 - 0.000124 = 2.249876 мин на одну заявку
2.249876 * 521 = 1172.19 мин = 19.54 ч за расчетный месяц
```

Как проектное допущение дальнейшего развития, для денежного расчета заданы следующие прозрачные параметры сценария: часовая ставка HR-специалиста `600` руб./ч; стоимость машинного времени CPU-инстанса `30` руб./ч; инженерная ставка для внедрения и сопровождения `1500` руб./ч; разовое внедрение готового прототипа в пилотный контур Reqcore занимает `16` инженерных часов; ежемесячное сопровождение занимает `2` инженерных часа. В прототипе стоимость первоначального обучения модели также включена в затраты на внедрение на основе внешнего замера команды обучения `6254.096` с, что при ставке `30` руб./ч дает `52.12` руб.

Формулы денежной оценки:

```text
Стоимость ручной обработки одной заявки = Время человека на одну заявку * Часовая ставка HR
Стоимость обработки после внедрения = Время модели на одну заявку * Стоимость машинного времени + Время проверки HR * Часовая ставка HR
Экономия на одной заявке = Стоимость ручной обработки одной заявки - Стоимость обработки после внедрения
Экономия за период = Экономия на одной заявке * Количество заявок за период
Чистый эффект за период = Экономия за период - Затраты на сопровождение за период
Срок окупаемости = Затраты на внедрение / Чистый ежемесячный эффект
ROI = (Чистый эффект за период - Затраты на внедрение) / Затраты на внедрение * 100%
```
...
В эмпирической части работы практическая ценность внедрения состоит в том, что модель сокращает первичный машинно-ручной цикл обработки с `2.25` минуты ручной работы до `0.20` минуты HR-проверки и `0.007429` секунды локального инференса на заявку в расчетном сценарии. При сохранении контрольной роли HR-специалиста качество модели на отложенном test split остается высоким: `accuracy = 0.9750`, `Average Precision = 0.9921`, `Precision@3 = 0.9958`. Следовательно, экономический эффект достигается не за счет исключения человека из процесса, а за счет переноса трудоемкого первичного просмотра на локальный модуль ранжирования с последующей быстрой проверкой результата.
