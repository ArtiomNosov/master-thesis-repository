...
**Проектирование архитектуры конвейера предварительной обработки (preprocessing pipeline) и модуля вычисления семантической близости**
Архитектура конвейера предварительной обработки (preprocessing pipeline) проектируется в виде логически упорядоченной последовательности модулей предпроцессинга:
...
**Разработка программного кода подготовки данных и конвейера машинного обучения (machine learning pipeline)**
Контур разработки ML-кода разделен на офлайн- и онлайн-контуры в соответствии с передовой практикой систем талантов [17]. Офлайн-модули собирают, подготавливают данные и проводят обучение; онлайн-система занимается инференсом эмбеддингов и передачей API-команд. Совокупный программный комплекс (ATS AI Engine) состоит из 18 взаимосвязанных Python-скриптов, исполняемых в строго определенном порядке нумерации: от модулей нормализации и извлечения сущностей (скрипты 05–07), через модули обучения и инференса bi-encoder модели (08–09), ядро ранжирования и API-микросервис (10–11), до серии экспериментальных бенчмарков (12–17) и отдельного контура обучения cross-encoder (18). Все зависимости фиксированы через стандартные библиотеки (`sentence-transformers`, `transformers`, `scikit-learn`, `fastapi`, `uvicorn`), что обеспечивает полную воспроизводимость результатов.
Конвейер машинного обучения (machine learning pipeline) включает подпрограммы для:
...
Модуль оценивался в формате офлайн-валидации на пересобранном наборе данных, сформированном из первоисточников агрегатора вакансий и резюме. После очистки и пересборки связанных пар итоговая выборка составила `5 218` примеров, из которых `4 176` использовались для обучения, `521` для валидации и `521` для финального тестирования. Основной сравнительный протокол включает ровно три модели: BM25 lexical ranking baseline без обучения; fine-tuned cross-encoder `cointegrated/rubert-tiny2`, обучаемый на `train.tsv` и валидируемый на `val.tsv`; fine-tuned bi-encoder `experiments/models/bi_encoder_rubert_tiny2`, выступающий proposed approach проекта.

На уровне отдельных пар `вакансия-резюме` качество оценивается по метрикам Average Precision, F1, Accuracy и MCC, а финальные значения публикуются только на `test.tsv`. Для BM25 score задается классической формулой релевантности между вакансией как query и резюме как document после регулярной токенизации по словам и lowercasing. Для cross-encoder входом является пара `[vacancy_text, resume_text]`, а score равен вероятности класса `label = 1`. Для bi-encoder вакансия и резюме кодируются независимо, а score рассчитывается как cosine similarity между embeddings. В имеющемся прогоне proposed bi-encoder после fine-tuning достиг на `val` Average Precision `0.9870`, best F1 `0.9582`, best accuracy `0.9712` и MCC `0.9365`; на `test` модель показала Average Precision `0.9921`, best F1 `0.9636`, best accuracy `0.9750` и MCC `0.9447`.
...
Таким образом, экспериментальная логика работы состоит не в переборе близких вариантов одной модели, а в сравнении трех разных компромиссов. BM25 показывает силу классического поиска по совпадению слов и терминов; cross-encoder показывает потенциальную верхнюю границу качества при дорогом совместном анализе каждой пары; bi-encoder проверяет, насколько масштабируемая семантическая архитектура конкурентоспособна и пригодна для интеграции в ATS. Полученные результаты следует интерпретировать как офлайн-валидацию на исторических данных, а не как онлайн-эксперимент в рабочем контуре ATS.

Ограничения экспериментальной части также должны быть зафиксированы явно. Сравнение BM25, cross-encoder и bi-encoder не заменяет HR-валидацию explainability, bias-audit по социальным признакам и нагрузочное сравнение больших кандидатских пулов. Кроме того, treatment-ветка `biencoder` в текущем коде возвращает только числовой score и не формирует criterion-level `evidence`, `strengths` и `gaps`, поэтому результаты корректно трактовать именно как проверку качества числового сопоставления и ранжирования, а не как оценку полноты объяснимости решения.
...
Для сценария предварительного top-K ранжирования через bi-encoder экономия внешних LLM-вызовов описывается так:
...
где `N` - число кандидатов в исходном пуле, `K` - число кандидатов, передаваемых на дальнейший LLM-анализ после предварительного ранжирования, `L_bi(N)` - задержка локального bi-encoder scoring для пула из `N` кандидатов, а `C_cpu_hour` - стоимость часа CPU-инстанса. Если инфраструктурная стоимость локального scoring мала относительно LLM-вызовов, то для `N = 100` и `K = 10` приближенная экономия внешних вызовов составляет около `1 - K / N = 90%`.

Задержка полного LLM scoring и bi-encoder pre-ranking задается следующими формулами:
...
где `L_llm_pair` - задержка одного LLM-запроса для пары «вакансия-кандидат», `P_parallel` - допустимое число параллельных LLM-запросов, `L_filter(N)` - время применения жестких фильтров, `L_encode_vacancy` - время кодирования вакансии, `L_encode_candidates(N)` - время кодирования кандидатов, `L_cosine_topK(N)` - время расчета косинусных сходств и выбора top-K. LLM scoring дает критериальное объяснимое оценивание, но зависит от внешнего API и переменной стоимости вызова. Bi-encoder подходит для первичного top-K отбора без внешних API-платежей, но требует локального CPU/RAM и не заменяет полную LLM-интерпретацию.
