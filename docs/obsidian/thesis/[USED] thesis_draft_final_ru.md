...
**Система обозначений статуса утверждений (SAT).** В тексте и на диаграммах различаются проектные архитектурные решения, реализованный прототип, подготовленные данные и экспериментально проверенные результаты. Для диаграмм используется единая цветовая шкала SAT (Status Assertion Taxonomy): базовая нотация процесса — BPMN 2.0, архитектуры — C4 Container (уровень контейнеров), потока интеграции — flowchart; цвет и тип рамки показывают эпистемический статус элемента (см. легенды на рисунках). В тексте статус передаётся через формулировки вроде «в прототипе реализовано», «экспериментально подтверждено», «как проектное допущение», а не через однотипные метки перед абзацем.
...
Целью работы является проектирование модуля сопоставления «вакансия–резюме» на основе семантических представлений текста и разработка архитектуры ранжирования кандидатов для интеграции в ATS. Сопутствующими задачами стали воспроизводимая методика оценки качества ранжирования (по метрикам NDCG, Precision@K и др.) и расчёт практической эффективности созданного прототипа. Методологическая база исследования включает современные нейросетевые модели контекстных представлений (BERT, Sentence-BERT), подходы на базе независимого энкодера (bi-encoder) и сиамских архитектур для поиска по семантическим вложениям, а также передовые практики индустриальных систем поиска талантов [1, 6, 8, 17].
...
С точки зрения целевой архитектуры, в сценарии разработки для ATS вышеупомянутые выводы делают рациональной реализацию двухступенчатого ранжирования. Первая ступень — стадия выборки (recall), основанная на поиске по плотным вложениям через би-энкодер для быстрого отбора топ-N кандидатов из глобальной базы. Вторая ступень — стадия переранжирования (re-rank), которая может осуществляться более ресурсоемкой моделью перекрестного энкодера либо строиться как отдельная LTR-надстройка над широким пулом признаков. В рамках данной работы эмпирически проверяется bi-encoder scoring и ranking-оценка текущего прототипа; полноценный cross-encoder/LTR re-rank слой остается проектным направлением, а не экспериментально подтвержденным результатом. Подобная логика построения конвейера явно задокументирована в передовых индустриальных исследованиях корпоративного поиска соискателей [17].
...
В эмпирической части работы в качестве экспериментальной базы использован корпус данных на основе одного из популярных сервисов по поиску работы, включающий массив резюме (~17,3 ГБ, содержащий профили кандидатов с полями профессиональной специализации, стажа и зарплатных ожиданий), массив описаний вакансий (~44,9 ГБ, включающий текстовые описания, требования, условия и метаданные), а также журнал событий откликов (~293 МБ), служащий ключевым источником для формирования обучающих пар «кандидат–вакансия». Помимо основных массивов корпус содержит дополнительные таблицы профессиональных навыков, опыта работы, образования и иных атрибутов кандидатов. В прототипе масштаб корпуса обусловил создание потокового конвейера обработки (streaming batch processing) с поддержкой чанкирования (chunking), обеспечивающего стабильную работу при строгих ограничениях оперативной памяти без загрузки полного объема информации в RAM.
...
Процедура разметки массива данных в рамках проекта протекает на нескольких уровнях, однако их статус различается:
1. В прототипе реализован разметка глобальной релевантности пар «вакансия–резюме» для последующего обучения ранжирующих моделей и контроля качества по NDCG.
2. На концептуальном уровне описан детализированная разметка терминов на уровне выделения текстовых фрагментов (span-level) с типизацией скиллов (hard, soft, knowledge) для построения интерпретируемой выдачи.
3. Для масштабирования допускается, что косвенная разметка посредством накопления поведенческих сигналов рекрутеров (отказы, приглашения на собеседование) рассматривается как элемент промышленной эксплуатации алгоритма, но в настоящей работе не проверяется как онлайн-механизм.
...
Ключевым примером при решении задачи разметки является ресурс SKILLSPAN, включающий более 14 тысяч предложений из описаний вакансий, экспертно размеченных согласно строгим гайдлайнам. В исследовании показано, что доменно адаптированные модели машинного обучения (single-task подход) в этой задаче значительно превосходят общие алгоритмы генерации текстов [24]. Архитектурно основываясь на этом, целесообразно актуализировать собственную таксономию навыков, выполнять настройку трансформеров на локальных корпусах ATS и оформлять модуль извлечения навыков как независимый подпроцесс [18, 24]. В текущем прототипе реализовано извлечение маркеров квалификации и технологических навыков, но полноценная экспертная span-level разметка навыков не проводилась.
...
С точки зрения целевой архитектуры, пайплайн проектируется в виде логически упорядоченной последовательности модулей предпроцессинга. Ниже отдельно обозначены компоненты, которые реализованы и проверены в прототипе, и компоненты, которые используются как проектная рамка дальнейшего развития:

1. В прототипе реализован модуль приема (ingestion) получает профили кандидатов и описания позиций, проверяет структурный формат и нормализует кодировки. В целевой ATS-системе (Reqcore) извлечение сырого текста из загруженных резюме осуществляется на основе MIME-типа файла с привлечением специализированных библиотек парсинга (PDF, DOCX, DOC), после чего применяется эвристическая сегментация на смысловые секции (опыт работы, образование, навыки) посредством регулярных выражений по характерным паттернам заголовков.
2. В прототипе реализован модуль лингвистической нормализации выполняет токенизацию строк, удаление пунктуации и приведение текста к нижнему регистру (lowercasing) с сохранением значимых IT-символов (например, `+`, `#` для обозначений C++, C#). Модуль выявляет язык источника, производит дедупликацию, удаляет юридические штампы и шаблонные HR-блоки (типовые фрагменты «Обязанности:», «Условия:», «Описание вакансии:») при помощи регулярных выражений для снижения шума в последующих эмбеддингах.
3. В прототипе реализован структурно-аналитический модуль вычленяет смысловые кластеры (опыт работы, образование, навыки) и специализированные термины по категориям hard/soft skills. В реализации модуль выполняет извлечение маркеров уровня квалификации (Intern/Junior, Middle, Senior, Lead) и ключевых технологических навыков (Python, SQL, Docker, Kubernetes и др.) с формированием структурированных JSON-списков. Формируется детальная цифровая матрица компетенций профиля [24].
4. В прототипе реализован модуль построения эмбеддингов конвертирует нормализованные тексты в плотные векторные представления на базе сиамской архитектуры Sentence-BERT. Перед подачей в энкодер извлеченные признаки агрегируются в унифицированный текстовый формат вида `Role: [роль] | Seniority: [уровень] | Skills: [список навыков] | Text: [очищенное описание]`. На отложенной выборке показано, что доменное дообучение би-энкодера на подготовленных парах проверено на val/test split. Архитектурное допущение о дальнейшем обогащении базовых весов ин-хаус используется как направление развития, а не как отдельный подтвержденный производственный контур [1, 6, 24].
5. Архитектурно блок первичной выборки (Recall/Retrieval) отбирает узкую первичную выборку лучших кандидатов по критериям семантической схожести векторов. В прототипе в текущем прототипе этот шаг выполнен как brute-force cosine scoring по in-memory пулу. Для масштабирования рассматривается допущение, что подсчет схожести через ANN и Vector DB (например, Milvus) рассматривается как направление масштабирования, но не является проверенным результатом текущего эксперимента [12].
6. Для масштабирования допускается, что модуль алгоритмического переранжирования (Re-ranking) на основе LTR-алгоритмов рассматривается как следующий этап развития, в котором могут интегрироваться исходные семантические скоры, структурные пересечения по профессиональному стажу и прокси-метрики поведения [17]. В текущем коде такой re-ranking слой не реализован.
7. На концептуальном уровне описан модуль объяснимости (Explainability Engine) описывает требование к интерпретируемым сводкам, например текстовым плашкам совпадения навыков. Механизм прозрачности работы модели призван компенсировать фактор эффекта «черного ящика», однако HR-валидация explainability в рамках текущей работы не выполнялась [23].
8. В прототипе реализован в целевой ATS-системе (Reqcore) существует автоматизированный конвейер LLM-генерации критериев оценки на основе текста описания вакансии. Система передает описание позиции в LLM (с поддержкой нескольких провайдеров: OpenAI, Anthropic, Google AI) и получает структурированный ответ, жестко валидируемый JSON-схемой. Каждый сгенерированный критерий содержит уникальный идентификатор, наименование, инструкцию по поиску фактов в резюме, категорию (технический навык, soft skill, образование), базовую шкалу оценки и рекомендованный вес значимости. Полученные критерии далее выступают в роли формализованного чек-листа для автоматического AI-скоринга входящих резюме.

Контейнерная архитектура текущего решения и проектных направлений развития представлена на рисунке 1.

![C4 Container диаграмма с пометками статуса компонентов](assets/c4_container_architecture_marked.svg)

Рисунок 1 — Диаграмма контейнеров C4 (нотация C4 Container + шкала SAT). Зелёная рамка — реализованные контейнеры прототипа; светло-зелёная — исследованные артефакты данных; бирюзовая — экспериментально проверенные блоки (модель, метрики); оранжевая пунктирная — архитектурные допущения; фиолетовая пунктирная — концептуальные элементы без эмпирической проверки. Технические идентификаторы (`application.score`, `NDCG@K`, `/score`) указаны в скобках после русских названий.

**Спецификация программного интерфейса (API) и модулей интеграции с ядром ATS**

В созданном прототипе API-модуль создан в скрипте API-сервера (`experiments/scripts/11_api_server.py`) как микросервис FastAPI `ATS Semantic Ranking Engine API` версии `1.0.0`. В текущей реализации сервер использует Pydantic-схемы `CandidatePayload` и `SearchPayload`, временный индекс кандидатов в памяти (`CANDIDATE_INDEX`) и модуль ранжирования (`ATSRanker`), загружаемый при старте приложения. Кандидатский профиль передается в API в уже подготовленном текстовом формате `model_input`; отдельные публичные маршруты для самостоятельного построения эмбеддингов вакансии или резюме в коде не реализованы.
...
В созданном прототипе текущая серверная интеграция между ATS Reqcore и модулем ML-оценки выполнена в обработчике `ats/reqcore/server/api/applications/[id]/analyze.post.ts`. Эта точка является фактическим входом для анализа одной заявки и отвечает за выбор ветки скоринга по значению провайдера (`ai_config.provider`). В обоих вариантах результат интеграции сводится к обновлению числового поля оценки заявки (`application.score`) и записи аудита анализа (`analysis_run`); различается только способ получения этого числа.

Основной путь Reqcore в текущей реализации является LLM-ориентированным. Обработчик `analyze.post.ts` загружает заявку, AI-конфигурацию организации, критерии оценки из `scoring_criterion`, текст резюме из `document.parsedContent`, затем вызывает функцию скоринга заявки (`scoreApplication()`) из `ats/reqcore/server/utils/ai/scoring.ts`. Функция `scoreApplication()` формирует структурированный prompt по вакансии, критериям и материалам кандидата и передает его в `generateStructuredOutput()` из `ats/reqcore/server/utils/ai/provider.ts`, где выбирается конкретный провайдер (`openai`, `anthropic`, `google` или `openai_compatible`). Возвращенный структурированный ответ содержит массив `evaluations` с полями `criterionKey`, `applicantScore`, `confidence`, `evidence`, `strengths` и `gaps`. После этого в `analyze.post.ts` вычисляется итоговый балл через `computeCompositeScore()`, записи по критериям сохраняются в таблицу оценок по критериям (`criterion_score`), а агрегированный результат записывается в поле оценки заявки (`application.score`).

Альтернативный путь интеграции реализован как ветка провайдера bi-encoder (`provider === 'biencoder'`) в том же файле `analyze.post.ts`. В этой ветке Reqcore не вызывает LLM-провайдер и не использует функцию скоринга заявки (`scoreApplication()`). Вместо этого обработчик обращается к локальной функции `scoreWithBiEncoder()`, которая читает URL сервиса bi-encoder (`BIENCODER_URL`) из окружения и отправляет HTTP POST на Python-микросервис из скрипта API-сервера (`experiments/scripts/11_api_server.py`) по маршруту оценки (`/score`) с телом `{ vacancy_text, candidate_text }`. Python-сервис загружает модуль ранжирования (`ATSRanker`) и напрямую вычисляет косинусное сходство (cosine similarity) между эмбеддингом вакансии и эмбеддингом одного резюме, после чего возвращает `match_score`. Затем `analyze.post.ts` линейно отображает этот score из диапазона `[-1, 1]` в шкалу `0..100` и записывает результат в поле оценки заявки (`application.score`). Для этой ветки массив `evaluations` остается пустым, а записи в `criterion_score` не создаются, поскольку bi-encoder путь в текущем коде предназначен для числового pre-ranking или альтернативного coarse scoring, а не для замены полного критериального LLM-анализа.

Следовательно, фактическая точка интеграции находится не в UI и не в клиентском коде, а в серверном маршруте `ats/reqcore/server/api/applications/[id]/analyze.post.ts`, где выбирается один из двух путей: `analyze.post.ts -> scoreApplication() -> provider -> criterion_score -> application.score` для критериального LLM-scoring и `analyze.post.ts -> biencoder branch -> HTTP /score -> numeric score -> application.score` для оценки через bi-encoder (маршрут `/score`, поле `application.score`). С точки зрения архитектуры bi-encoder путь следует описывать как альтернативный или предварительный путь ранжирования, пригодный для дешевого числового отбора, но не как полную замену всей LLM-логики Reqcore, поскольку в нем отсутствуют criterion-level explanations, evidence, strengths/gaps и генерация структурированной интерпретации.

Компактная схема развертывания текущей интеграции имеет вид:

![Схема потока анализа заявки с пометками статуса компонентов](assets/application_analysis_request_flow_marked.svg)

Рисунок 2 — Потоковая схема анализа заявки (flowchart + SAT). Зелёная рамка — реализованные шаги Reqcore и FastAPI; бирюзовая — экспериментально проверенный расчёт косинусной оценки (`ATSRanker`, val/test split); синяя зона у ромба — архитектурный выбор ветки LLM-scoring или bi-encoder (`provider === 'biencoder'`, `/score`); фиолетовая — концептуальное ограничение ветки «только score».

В созданном прототипе в текущем контуре затронуты следующие файлы: `ats/reqcore/server/api/applications/[id]/analyze.post.ts` как основная точка маршрутизации; `ats/reqcore/server/utils/ai/scoring.ts` как движок LLM-scoring; `ats/reqcore/server/utils/ai/provider.ts` как адаптер провайдера (`provider`); `ats/reqcore/server/database/schema/app.ts` как место определения таблиц оценок по критериям (`criterion_score`), аудита анализа (`analysis_run`) и поля оценки заявки (`application.score`); `experiments/scripts/11_api_server.py` как Python-микросервис bi-encoder scoring. Именно эти файлы образуют фактическую интеграционную поверхность между Reqcore и модулем ML-ранжирования.
...
С точки зрения целевой архитектуры, контур разработки ML-кода разделен на офлайн- и онлайн-контуры в соответствии с передовой практикой систем талантов [17]. Офлайн-модули собирают, подготавливают данные и проводят обучение; онлайн-система занимается инференсом эмбеддингов и передачей API-команд. В прототипе совокупный программный комплекс (ATS AI Engine) состоит из 17 взаимосвязанных Python-скриптов, исполняемых в строго определенном порядке нумерации: от модулей нормализации и извлечения сущностей (скрипты 05–07), через модули обучения и инференса модели (08–09), ядро ранжирования и API-микросервис (10–11), до серии экспериментальных бенчмарков (12–17). Все зависимости фиксированы через стандартные библиотеки (`sentence-transformers`, `scikit-learn`, `fastapi`, `uvicorn`), что обеспечивает полную воспроизводимость результатов.
...
В созданном прототипе при реализации контура векторного кодирования фактически используется только модель `cointegrated/rubert-tiny2`. Это подтверждается обучающим скриптом `experiments/scripts/09_train_biencoder.py`, где аргумент `--model_name` имеет значение по умолчанию `cointegrated/rubert-tiny2`, а также сохраненным артефактом `experiments/models/bi_encoder_rubert_tiny2`, в `README.md` которого указан `base_model: cointegrated/rubert-tiny2`. Для масштабирования рассматривается допущение, что возможное применение `ru-en-RoSBERTa` рассматривается как направление дальнейшего сравнения, но не как результат текущего эксперимента.
...
В созданном прототипе алгоритм ранжирования соискателей создан в классе `ATSRanker` файла `experiments/scripts/10_ranking_module.py`. Текущая реализация использует последовательный конвейер из трех операций: жесткая фильтрация кандидатского пула, кодирование текста вакансии и резюме через `SentenceTransformer`, затем вычисление косинусного сходства и сортировка результатов по убыванию score. Для масштабирования рассматривается допущение, что Learning to Rank, RankNet, Milvus, DiskANN, SPANN, отдельный ANN-индекс и модуль справедливого ранжирования относятся к будущему развитию и не описываются как реализованные элементы текущей работы.
...
В созданном прототипе интеграция выполнена в виде отдельного FastAPI-сервиса ранжирования, который взаимодействует с ATS по HTTP. В текущем коде Python-сервис из скрипта API-сервера (`experiments/scripts/11_api_server.py`) предоставляет три маршрута: `POST /index/candidate` для загрузки кандидатских профилей во временный in-memory пул, `POST /search/rank` для ранжирования списка кандидатов по косинусному сходству (cosine similarity) и `POST /score` (маршрут оценки) для расчета score одной пары `вакансия-кандидат`. Таким образом, фактически реализованный контур представляет собой внешний сервис семантического скоринга с in-memory хранением профилей, а не связку с промышленной векторной базой данных.

В эмпирической части работы работоспособность API-контура подтверждена отдельным end-to-end тестом `experiments/scripts/16_e2e_integration_test.py`. В этом сценарии через `FastAPI TestClient` последовательно проверяются загрузка кандидатских профилей через `POST /index/candidate`, ранжирование через `POST /search/rank` и одиночную оценку пары через `POST /score` (маршрут оценки). Тест валидирует HTTP-ответы, JSON-контракт и наличие телеметрических заголовков `X-Processing-Time-Ms` и `X-Model-Version`. Следовательно, на уровне тестового контура подтверждено, что сервис корректно обрабатывает ingest, ranking и single-score запросы в рамках одного API.

Дополнительно была выполнена практическая локальная проверка интеграции с Reqcore. Для этого был поднят локальный demo-стенд Reqcore через Docker Compose, Python bi-encoder сервис был запущен отдельно с указанием `MODEL_PATH`, а серверная переменная `BIENCODER_URL` была направлена на этот сервис. В результате treatment-путь провайдера bi-encoder (`provider === 'biencoder'`) в `ats/reqcore/server/api/applications/[id]/analyze.post.ts` был реально пройден до конца: Reqcore вызвал `POST /score` (маршрут оценки), получил числовой `match_score`, преобразовал его в итоговое поле оценки заявки (`application.score`) и сохранил завершённый аудит анализа (`analysis_run`) с `provider = biencoder`, `model = cointegrated/rubert-tiny2`, `promptTokens = 0` и `completionTokens = 0`. Тем самым подтверждена не только автономная работа FastAPI-сервиса, но и его фактическое подключение к ATS-контру Reqcore.

Важно, что текущая интеграция с Reqcore не заменяет весь LLM-контур целиком, а добавляет альтернативный score-only путь. Для стандартного LLM-сценария маршрут `POST /api/applications/[id]/analyze` продолжает использовать `scoreApplication()`, провайдера LLM и сохранение criterion-level breakdown в `criterion_score`. Для biencoder-сценария тот же маршрут использует отдельную ветку с вызовом URL сервиса bi-encoder (`BIENCODER_URL`) и маршрута оценки (`/score`), после чего в ATS записывается только итоговый числовой score без оценок по критериям (`criterion_score`), `evidence`, `strengths` и `gaps`. Следовательно, интеграция корректно описывается как подключение альтернативного семантического scoring-модуля, а не как внедрение полноценной векторной БД или полного replacement всего LLM-анализа.

Ограничения интеграционного контура также должны быть зафиксированы явно. В текущей реализации не подтверждены утверждения о корпоративном дисковом векторном хранилище профилей, Milvus, DiskANN, SPANN, миллионных индексах, графовой навигации по ANN-структурам или специальных механизмах контроля деградации через версионируемое внешнее векторное хранилище. Также не подтверждено наличие отдельного production-контура аудита на независимых тестовых корпусах русского языка. Реально реализованный и проверенный контур ограничивается bi-encoder микросервисом, in-memory candidate pool для API ранжирования, Reqcore-интеграцией через URL сервиса (`BIENCODER_URL`) и локальным E2E/smoke-тестированием этой связки.

**Экономическая оценка LLM scoring и bi-encoder scoring**

Экономическая оценка не опирается на production-логи токенов и не утверждает, что фактический расход или замер рабочего времени HR извлечены из промышленной ATS. В коде Reqcore предусмотрены поля `analysis_run.prompt_tokens`, `analysis_run.completion_tokens` и настраиваемые `input_price_per_1m` / `output_price_per_1m`; UI рассчитывает стоимость по формуле `(promptTokens / 1_000_000) * inputPrice + (completionTokens / 1_000_000) * outputPrice`. В рамках работы эти значения из production-БД не выгружались, поэтому ниже — оценочный расчёт на публичных pricing constants, измеренном времени инференса и явных допущениях.

Используемые публичные константы цен: для `gpt-4o-mini` (default в `ai_config`) — `$0.15` / 1 млн input и `$0.60` / 1 млн output tokens; для `gpt-5.4-mini` — `$0.75`, `$0.075` (cached) и `$4.50`; для `gpt-5.4-nano` — `$0.20`, `$0.02` и `$1.25`. Cached input в расчётах не используется: применение prompt caching для scoring в коде не подтверждено.

Формула стоимости одного LLM scoring-запроса для пары «вакансия–кандидат»:

```text
C_llm_pair = (T_in / 1_000_000) * P_in + (T_out / 1_000_000) * P_out
```

где `T_in` и `T_out` — число input/output tokens, `P_in` и `P_out` — публичные цены за 1 млн tokens. Оценочное допущение: `T_in = 3 000`, `T_out = 700`, один LLM-вызов на заявку.

| Модель | Estimate cost / application | Estimate cost / 100 applications |
|---|---:|---:|
| `gpt-4o-mini` | `$0.00087` | `$0.087` |
| `gpt-5.4-mini` | `$0.00540` | `$0.540` |
| `gpt-5.4-nano` | `$0.001475` | `$0.1475` |

Стоимость bi-encoder scoring: модуль ранжирования (`ATSRanker`) не вызывает внешний LLM API, а локально кодирует тексты через `SentenceTransformer`. Внешний API-cost:
...
где `N` — число кандидатов в пуле, `L_bi(N)` — задержка локального scoring (секунды), `C_cpu_hour` — стоимость CPU-инстанса ($/ч). Production benchmark `L_bi(N)` и `C_cpu_hour` в работе не измерялись.

Формула top-K pre-ranking:
...
При `N = 100`, `K = 10` сокращение внешних LLM-вызовов — `90%`. Задержка LLM:
...
LLM scoring даёт критериальное объяснимое оценивание, но зависит от внешнего API и переменной стоимости вызова. Bi-encoder подходит для первичного top-K отбора без внешних API-платежей, но требует локального CPU/RAM и не заменяет полную LLM-интерпретацию.

**Расчётный сценарий экономии времени HR**

В эмпирической части источник модельного времени — прогон на `data/splits/test.tsv`: `experiments/results/full_test_ranking_metrics_stdout.txt` фиксирует `521` пару и `runtime_seconds = 3.870512`.

```text
T_model = 3.870512 / 521 = 0.007429 с = 0.000124 мин.
```

Для ручного сценария (допущение): `45` мин на `20` заявок → `T_human = 2.25` мин/заявку. После внедрения HR проверяет ранжированный список: `4` мин на `20` заявок → `T_check_hr = 0.20` мин.

| Показатель | Ручная обработка | Модельный контур | Разница |
|---|---:|---:|---:|
| Время на одну заявку | `2.25` мин | `0.000124` мин инференса | `2.249876` мин |
| Время HR после внедрения | `2.25` мин | `0.20` мин проверки | экономия `2.05` мин |
| Accuracy к HR-меткам | `1.0000` | `0.9750` | `-0.0250` |
| Average Precision (test) | — | `0.9921` | модельная метрика |
| Precision@3 (test) | — | `0.9958` | модельная метрика |

За месяц принято `521` заявка (объём test split). Экономия времени:

```text
Экономия на заявке = 2.25 - 0.000124 = 2.249876 мин
Экономия за период = 2.249876 * 521 = 1172.19 мин ≈ 19.54 ч
```

Параметры денежного сценария (допущения): HR `600` руб./ч, CPU `30` руб./ч, внедрение `16` ч × `1500` руб./ч, сопровождение `2` ч/мес, обучение модели `52.12` руб.

| Метрика | Значение |
|---|---:|
| Стоимость ручной обработки / заявка | `22.50` руб. |
| Стоимость после внедрения / заявка | `2.0001` руб. |
| Экономия / заявка | `20.50` руб. |
| Экономия / месяц | `10680.47` руб. |
| Чистый эффект / месяц | `7680.47` руб. |
| Срок окупаемости | `3.13` мес. |
| ROI за 12 мес. | `283.2%` |

Практический эффект: перенос трудоёмкого первичного просмотра на локальный модуль ранжирования при сохранении контрольной роли HR и высоком качестве на test split (`accuracy = 0.9750`, `AP = 0.9921`, `Precision@3 = 0.9958`).


**Офлайн-валидация bi-encoder-scoring в Reqcore**

Экспериментальная проверка подтвердила, что в данной работе вместо продуктового A/B-теста используется офлайн-оценка качества модуля bi-encoder-scoring на фиксированном отложенном наборе данных. Такой выбор соответствует текущей архитектуре Reqcore: AI-конфигурация хранится на уровне организации в `ai_config`, а маршрут `POST /api/applications/[id]/analyze` при каждом запуске анализа читает один активный `provider` и одну активную `model` для всей выбранной организации. Следовательно, в текущей реализации без дополнительного server-side механизма назначения experimental arm невозможно корректно провести одновременный A/B-тест внутри одной и той же организации на уровне отдельных вакансий или заявок. Поэтому наиболее корректным и простым способом показать эффективность является офлайн-сравнение качества скоринга на заранее сформированных `train/val/test` split-файлах.

В качестве основного сценария оценки используется сравнение базовой модели `cointegrated/rubert-tiny2` без дообучения и дообученной версии той же модели, которая затем применяется в ветке провайдера bi-encoder (`provider === 'biencoder'`) при интеграции с Reqcore. Для каждой пары `вакансия-резюме` вычисляется cosine similarity между эмбеддингом вакансии и эмбеддингом резюме, после чего качество оценивается на отложенной выборке. Такой дизайн непосредственно соответствует текущей логике модуля ранжирования: hard filtering, embedding encoding и cosine scoring, а также числовому полю оценки заявки (`application.score`), которое записывается в Reqcore через biencoder-ветку.
...
На уровне отдельных пар используются метрики бинарного разделения, прежде всего Average Precision, F1, Accuracy и MCC. Они показывают, насколько хорошо cosine score различает положительные и отрицательные пары `вакансия-резюме`. На уровне ранжирования внутри вакансии используются метрики Precision@K, Recall@K, MRR и NDCG на K (`NDCG@K`) после группировки по `vacancy_id` и сортировки кандидатов по косинусной оценке (cosine score). Именно этот уровень оценки наиболее близок к реальному использованию модуля как инструмента предварительного отбора и сортировки кандидатов.
...
В созданном прототипе прототип решает задачу семантического сопоставления «вакансия-резюме» через би-энкодер `cointegrated/rubert-tiny2`, жесткую фильтрацию по metadata и косинусное ранжирование отфильтрованного кандидатского пула. Практический результат ограничен фактически реализованным контуром: подготовка связанных пар, обучение би-энкодера, сохранение модели, in-memory API и brute-force cosine scoring в `ATSRanker`. На отложенной выборке показано, что после повторной подготовки данных и fine-tuning метрики разделения пар на `val/test` существенно выросли относительно базовой модели; на полном `test.tsv` по группам вакансий также получены ranking-метрики: NDCG@3 `1.0000` на `33` NDCG-пригодных вакансиях, Precision@3 `0.9958`, Recall@3 `0.9824` и MRR `1.0000` на `119` вакансиях с хотя бы одним positive. Эти результаты подтверждают полезность доменной настройки для текущего корпуса, но ограничены разреженностью test split. Для масштабирования рассматривается допущение, что масштабирование через ANN/Vector DB, LTR/RankNet, fair ranking и промышленное нагрузочное тестирование в текущей реализации отсутствуют и должны рассматриваться как будущая работа.
