...
После внедрения модуль не принимает кадровое решение автономно: он сортирует заявки и передает результат HR-специалисту. Для контрольной проверки ранжированного списка принята верхняя граница из презентационного сценария: `4` минуты на `20` заявок, то есть:

```text
T_check_hr = 4 / 20 = 0.20 мин.
```

Сравнение по времени и качеству приведено в таблице. Точность ручной классификации относительно исторических HR-меток принимается равной `1.0000`, так как сами HR-события являются источником разметки `label`. Это не утверждение об абсолютной объективности человека, а техническое свойство выбранного офлайн-дизайна эксперимента. Для модели используется финальная test-метрика `best accuracy = 0.9750`, приведенная выше в разделе экспериментальной оценки.
...
Количество заявок за расчетный месяц принято равным `521`, то есть объему отложенного test split. Такой выбор не переносит тестовый набор в production как факт нагрузки, но позволяет сделать воспроизводимый расчет на тех же данных, на которых измерены качество и время. Тогда экономия времени по формуле:
