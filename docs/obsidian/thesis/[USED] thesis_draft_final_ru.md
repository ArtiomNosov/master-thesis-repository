...
Классическим программным контуром управления наймом выступают системы автоматизированного управления подбором персонала (Applicant Tracking System), которые обеспечивают хранение профилей кандидатов, управление стадиями воронки, а также выдачу упорядоченных списков кандидатов для рассмотрения рекрутером [2].
На практике задача ранжирования кандидатов в таких системах формулируется как задача информационного поиска и рекомендаций: в качестве запроса выступает описание вакансии или формализованные требования к ней, а документами являются резюме или профили кандидатов, которые необходимо отсортировать по степени релевантности [2, 3].
...
В сценарии разработки для системы автоматизированного подбора персонала вышеупомянутые выводы делают рациональной реализацию архитектуры двухступенчатого ранжирования. Первая ступень — стадия выборки (recall), основанная на поиске по плотным вложениям через независимый энкодер (bi-encoder) для быстрого отбора топ-N кандидатов из глобальной базы. Вторая ступень — стадия переранжирования (re-rank), которая может осуществляться более ресурсоемкой моделью перекрестного энкодера либо строиться как отдельная LTR-надстройка над широким пулом признаков. Подобная логика построения конвейера явно задокументирована в передовых индустриальных исследованиях корпоративного поиска соискателей [17].
...
Одним из таких направлений может стать введение отдельных публичных маршрутов для работы с эмбеддингами вакансий и резюме. В отличие от текущей реализации, где кандидатский профиль поступает в систему уже в подготовленном текстовом виде, выделение специализированных endpoint’ов для получения векторных представлений позволило бы формализовать этап предрасчёта признаков, упростить инкрементальное обновление представлений и сделать API более пригодным для сценариев, в которых этап подготовки данных отделён от этапа ранжирования. Такое расширение особенно актуально при переходе от прототипной схемы взаимодействия к более устойчивой сервисной архитектуре.
...
:Reqcore UI / recruiting system client sends request;
...
**3. Разработка и реализация модуля сопоставления вакансий и резюме для ранжирования кандидатов**

**Разработка программного кода подготовки данных и ML-пайплайна**
Программная реализация ML-модуля разделена на офлайн-часть и онлайн-часть в соответствии с передовой практикой систем поиска талантов [17]. Офлайн-модули собирают, очищают и размечают данные, формируют обучающие пары «текст вакансии–текст резюме» и проводят обучение модели; онлайн-часть выполняет инференс векторных представлений и предоставляет API-команды для ранжирования. Совокупный программный комплекс состоит из 18 взаимосвязанных Python-скриптов, исполняемых в строго определенном порядке нумерации: от модулей нормализации и извлечения сущностей (скрипты 05–07), через модули обучения и инференса модели независимого энкодера (bi-encoder) (08–09), ядро ранжирования и API-микросервис (10–11), до серии экспериментальных бенчмарков (12–17) и отдельного контура обучения cross-encoder (18). Все зависимости фиксированы через стандартные библиотеки (`sentence-transformers`, `transformers`, `scikit-learn`, `fastapi`, `uvicorn`), что обеспечивает полную воспроизводимость результатов.
...
Оценка качества текущей модели независимого энкодера (bi-encoder) интерпретируется как проверка семантического скоринга на сформированных split-файлах, а не как внешняя бизнес-валидация системы автоматизированного подбора персонала. Для исправленных `val.tsv` и `test.tsv` были измерены метрики бинарного разделения пар по cosine score с подбором порога на валидационном срезе. После fine-tuning сохраненная модель `experiments/models/bi_encoder_rubert_tiny2` достигла на `val` Average Precision `0.9870`, best F1 `0.9582`, best accuracy `0.9712`, MCC `0.9365`; на `test` модель показала Average Precision `0.9921`, best F1 `0.9636`, best accuracy `0.9750`, MCC `0.9447`. Эти числа подтверждают разделимость положительных и отрицательных пар для proposed approach, но не являются метриками fair ranking или внешней экспертной проверки.
...
Модуль оценивался в формате офлайн-валидации на пересобранном наборе данных, сформированном из первоисточников агрегатора вакансий и резюме. После очистки и пересборки связанных пар итоговая выборка составила `5 218` примеров, из которых `4 176` использовались для обучения, `521` для валидации и `521` для финального тестирования. Основной сравнительный протокол включает ровно три модели: BM25 lexical ranking baseline без обучения; fine-tuned cross-encoder `cointegrated/rubert-tiny2`, обучаемый на `train.tsv` и валидируемый на `val.tsv`; настроенный независимый энкодер (bi-encoder) `experiments/models/bi_encoder_rubert_tiny2`, выступающий proposed approach проекта.

На уровне отдельных пар `вакансия-резюме` качество оценивается по метрикам Average Precision, F1, Accuracy и MCC, а финальные значения публикуются только на `test.tsv`. Для BM25 score задается классической формулой релевантности между вакансией как query и резюме как document после регулярной токенизации по словам и lowercasing. Для cross-encoder входом является пара `[vacancy_text, resume_text]`, а score равен вероятности класса `label = 1`. Для независимого энкодера (bi-encoder) вакансия и резюме кодируются раздельно, а score рассчитывается как cosine similarity между embeddings. В имеющемся прогоне proposed approach после fine-tuning достиг на `val` Average Precision `0.9870`, best F1 `0.9582`, best accuracy `0.9712` и MCC `0.9365`; на `test` модель показала Average Precision `0.9921`, best F1 `0.9636`, best accuracy `0.9750` и MCC `0.9447`.
...
Таким образом, экспериментальная логика работы состоит не в переборе близких вариантов одной модели, а в сравнении трех разных компромиссов. BM25 показывает силу классического поиска по совпадению слов и терминов; cross-encoder показывает потенциальную верхнюю границу качества при дорогом совместном анализе каждой пары; независимый энкодер (bi-encoder) проверяет, насколько масштабируемая семантическая архитектура конкурентоспособна и пригодна для интеграции в систему автоматизированного управления подбором персонала. Полученные результаты следует интерпретировать как офлайн-валидацию на исторических данных, а не как онлайн-эксперимент в рабочем контуре Reqcore.

Ограничения экспериментальной части также должны быть зафиксированы явно. Сравнение BM25, cross-encoder и независимого энкодера (bi-encoder) не заменяет HR-валидацию explainability, bias-audit по социальным признакам и нагрузочное сравнение больших кандидатских пулов. Кроме того, treatment-ветка `biencoder` в текущем коде возвращает только числовой score и не формирует criterion-level `evidence`, `strengths` и `gaps`, поэтому результаты корректно трактовать именно как проверку качества числового сопоставления и ранжирования, а не как оценку полноты объяснимости решения.

**Выполнение интеграции созданного модуля в целевую систему автоматизированного управления подбором персонала или ее тестовый контур**

Интеграция реализована в виде отдельного сервиса HTTP API на Python-фреймворке FastAPI, который взаимодействует с Reqcore по HTTP. В текущем коде Python-сервис `experiments/scripts/11_api_server.py` предоставляет три маршрута: `POST /index/candidate` для загрузки кандидатских профилей во временный in-memory пул, `POST /search/rank` для ранжирования списка кандидатов по косинусному сходству и `POST /score` для расчета числовой оценки соответствия одной пары «текст вакансии–текст резюме». Таким образом, фактически реализованная интеграция представляет собой внешний сервис семантического скоринга с in-memory хранением профилей, а не связку с промышленной векторной базой данных.
...
Для сценария предварительного top-K ранжирования через независимый энкодер (bi-encoder) экономия внешних LLM-вызовов описывается так:
...
где `N` - число кандидатов в исходном пуле, `K` - число кандидатов, передаваемых на дальнейший LLM-анализ после предварительного ранжирования, `L_bi(N)` - задержка локального scoring независимого энкодера для пула из `N` кандидатов, а `C_cpu_hour` - стоимость часа CPU-инстанса. Если инфраструктурная стоимость локального scoring мала относительно LLM-вызовов, то для `N = 100` и `K = 10` приближенная экономия внешних вызовов составляет около `1 - K / N = 90%`.

Задержка полного LLM scoring и предварительного ранжирования независимым энкодером задается следующими формулами:
...
где `L_llm_pair` - задержка одного LLM-запроса для пары «вакансия-кандидат», `P_parallel` - допустимое число параллельных LLM-запросов, `L_filter(N)` - время применения жестких фильтров, `L_encode_vacancy` - время кодирования вакансии, `L_encode_candidates(N)` - время кодирования кандидатов, `L_cosine_topK(N)` - время расчета косинусных сходств и выбора top-K. LLM scoring дает критериальное объяснимое оценивание, но зависит от внешнего API и переменной стоимости вызова. Независимый энкодер подходит для первичного top-K отбора без внешних API-платежей, но требует локального CPU/RAM и не заменяет полную LLM-интерпретацию.

Расчет внедрения строится от измеримых показателей качества и времени. В качестве технического источника времени используется полный прогон ranking-оценки на `data/splits/test.tsv`: файл `experiments/results/full_test_ranking_metrics_stdout.txt` фиксирует `521` обработанную пару `вакансия-резюме` и `runtime_seconds = 3.870512`. Отсюда среднее модельное время на одну заявку:
...
Для ручного сценария в расчет введены явные допущения, поскольку в production-логах Reqcore отсутствует независимый замер рабочего времени HR-специалиста. В презентации ВКР для ручного скрининга уже зафиксирован ориентир `45` минут на цикл из `20` заявок; для расчета принято, что один такой цикл содержит ровно `20` заявок. После внедрения модуль не принимает кадровое решение автономно: он сортирует заявки и передает результат HR-специалисту. Для контрольной проверки ранжированного списка принята верхняя граница из презентационного сценария: `4` минуты на `20` заявок.
...
|---|---:|---:|---:|
| Время на одну заявку | `2.25` мин | `0.000124` мин модельного инференса | `2.249876` мин |
...
Для денежного расчета заданы следующие прозрачные допущения: часовая ставка HR-специалиста `600` руб./ч; стоимость машинного времени CPU-инстанса `30` руб./ч; инженерная ставка для внедрения и сопровождения `1500` руб./ч; разовое внедрение готового прототипа в пилотный контур Reqcore занимает `16` инженерных часов; ежемесячное сопровождение занимает `2` инженерных часа. Стоимость первоначального обучения модели также включена в затраты на внедрение: внешний замер команды обучения равен `6254.096` с, что при ставке `30` руб./ч дает `52.12` руб.
...
Практическая ценность внедрения состоит в том, что модель сокращает первичный машинно-ручной цикл обработки с `2.25` минуты ручной работы до `0.20` минуты HR-проверки и `0.007429` секунды локального инференса на заявку. При сохранении контрольной роли HR-специалиста качество модели на отложенном test split остается высоким: `accuracy = 0.9750`, `Average Precision = 0.9921`, `Precision@3 = 0.9958`. Следовательно, экономический эффект достигается не за счет исключения человека из процесса, а за счет переноса трудоемкого первичного просмотра на локальный модуль ранжирования с последующей быстрой проверкой результата.

**Офлайн-валидация трех стратегий ранжирования**

В данной работе вместо продуктового A/B-теста используется офлайн-оценка качества трех стратегий ранжирования на фиксированном отложенном наборе данных. Такой выбор соответствует текущей архитектуре Reqcore: AI-конфигурация хранится на уровне организации в `ai_config`, а маршрут `POST /api/applications/[id]/analyze` при каждом запуске анализа читает один активный `provider` и одну активную `model` для всей выбранной организации. Следовательно, в текущей реализации без дополнительного server-side механизма назначения experimental arm невозможно корректно провести одновременный A/B-тест внутри одной и той же организации на уровне отдельных вакансий или заявок. Поэтому наиболее корректным и простым способом показать эффективность является офлайн-сравнение BM25, cross-encoder и независимого энкодера (bi-encoder) на заранее сформированных `train/val/test` split-файлах.

В качестве основного сценария оценки используется сравнение ровно трех моделей. BM25 не обучается и проверяет силу лексического совпадения терминов, навыков и должностей. Cross-encoder `cointegrated/rubert-tiny2` обучается как sequence classification model для бинарной релевантности пары и возвращает вероятность `label = 1` для входа `[vacancy_text, resume_text]`. Независимый энкодер (bi-encoder) `experiments/models/bi_encoder_rubert_tiny2` кодирует вакансию и резюме раздельно, после чего вычисляет cosine similarity; именно эта логика соответствует текущей ветке `provider === 'biencoder'` и числовому `application.score`, который записывается в Reqcore.

Преимущество такого подхода состоит в том, что он не требует нового онлайн-эксперимента на пользователях и не требует обязательного привлечения дополнительных экспертных разметчиков. В качестве источника истинности используются уже подготовленные исторические пары `resume-vacancy` с метками, полученными из исходных HR-событий и затем разделенные на `train`, `val` и `test`. Это позволяет проверить не только абсолютное качество моделей, но и рациональность компромисса между качеством, скоростью, стоимостью inference и пригодностью для системы автоматизированного подбора персонала.

Офлайн-оценку целесообразно разделить на два уровня.

| Уровень оценки | Что измеряется | Артефакты |
|---|---|---|
| Pair-level classification | Насколько хорошо cross-encoder и независимый энкодер (bi-encoder) отделяют релевантные пары от нерелевантных | `train.tsv`, `val.tsv`, `test.tsv` |
| Vacancy-level ranking | Насколько хорошо BM25, cross-encoder и независимый энкодер (bi-encoder) ранжируют кандидатов внутри одной вакансии | группировка `test.tsv` по `vacancy_id` |

На уровне отдельных пар используются метрики бинарного разделения, прежде всего Average Precision, F1, Accuracy и MCC. Они показывают, насколько хорошо probability score cross-encoder и cosine score независимого энкодера различают положительные и отрицательные пары `вакансия-резюме`. На уровне ранжирования внутри вакансии используются метрики Precision@K, Recall@K, MRR и NDCG@K после группировки по `vacancy_id` и сортировки кандидатов по score каждой модели. Именно этот уровень оценки наиболее близок к реальному использованию модуля как инструмента предварительного отбора и сортировки кандидатов.

Минимальная процедура эксперимента выглядит следующим образом: обучаемые модели используют `train.tsv`, пороговые и вспомогательные параметры подбираются на `val.tsv`, а финальные метрики публикуются только на `test.tsv`, который не используется ни для обучения, ни для подбора решений. Для ranking-оценки каждая вакансия рассматривается как отдельный запрос, а все связанные с ней резюме сортируются по BM25 score, cross-encoder probability или cosine similarity независимого энкодера. Такой дизайн позволяет получить воспроизводимую и технически корректную оценку качества без вмешательства в рабочий продуктовый контур Reqcore.
...
Реализованный прототип решает практическую задачу предварительного отбора: он сравнивает описание вакансии и резюме, переводит оба текста в плотные семантические векторные представления и ранжирует профили по степени соответствия требованиям вакансии. Основой решения является архитектура независимого энкодера (bi-encoder) на базе `cointegrated/rubert-tiny2`: вакансия и резюме кодируются раздельно, после чего оценка рассчитывается по косинусному сходству. Практическая реализация включает подготовку связанных обучающих примеров «текст вакансии–текст резюме», обучение модели, сохранение артефакта, модуль `ATSRanker`, сервис HTTP API на Python-фреймворке FastAPI и подключение к тестовому контуру Reqcore, рассматриваемому как пример системы автоматизированного управления подбором персонала (Applicant Tracking System). Экспериментальный протокол сформулирован как сравнение ровно трех стратегий: BM25 проверяет классическое лексическое ранжирование без обучения, cross-encoder проверяет верхнюю границу качества при дорогом pairwise scoring, а независимый энкодер (bi-encoder) проверяет масштабируемое семантическое извлечение и сортировку кандидатов. Для текущей модели независимого энкодера на полном `test.tsv` по группам вакансий получены ranking-метрики: NDCG@3 `1.0000` на `33` NDCG-пригодных вакансиях, Precision@3 `0.9958`, Recall@3 `0.9824` и MRR `1.0000` на `119` вакансиях с хотя бы одним positive. Эти результаты подтверждают полезность доменной настройки для текущего корпуса, но ограничены разреженностью test split. Масштабирование через ANN/Vector DB, LTR/RankNet, fair ranking и промышленное нагрузочное тестирование в текущей реализации отсутствуют и должны рассматриваться как будущая работа.
